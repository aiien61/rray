# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

rray_bind_assign_cpp <- function(out, dim, args, lst_of_range_lsts, lst_of_arg_dim) {
    .Call(`_rray_rray_bind_assign_cpp`, out, dim, args, lst_of_range_lsts, lst_of_arg_dim)
}

rray_ones_cpp <- function(shape) {
    .Call(`_rray_rray_ones_cpp`, shape)
}

rray_zeros_cpp <- function(shape) {
    .Call(`_rray_rray_zeros_cpp`, shape)
}

rray_eye_cpp <- function(shape, k = 0L) {
    .Call(`_rray_rray_eye_cpp`, shape, k)
}

rray_eye_square_cpp <- function(n, k = 0L) {
    .Call(`_rray_rray_eye_square_cpp`, n, k)
}

rray_op_binary_cpp <- function(op, x, y) {
    .Call(`_rray_rray_op_binary_cpp`, op, x, y)
}

rray_op_trinary_cpp <- function(op, x, y, z) {
    .Call(`_rray_rray_op_trinary_cpp`, op, x, y, z)
}

rray_op_unary_one_cpp <- function(op, x, arg) {
    .Call(`_rray_rray_op_unary_one_cpp`, op, x, arg)
}

rray_op_unary_three_cpp <- function(op, x, arg1, arg2, arg3) {
    .Call(`_rray_rray_op_unary_three_cpp`, op, x, arg1, arg2, arg3)
}

rray_op_unary_two_cpp <- function(op, x, arg1, arg2) {
    .Call(`_rray_rray_op_unary_two_cpp`, op, x, arg1, arg2)
}

rray_op_unary_cpp <- function(op, x) {
    .Call(`_rray_rray_op_unary_cpp`, op, x)
}

rray_reducer_cpp <- function(op, x, axes) {
    .Call(`_rray_rray_reducer_cpp`, op, x, axes)
}

rray_subset_assign_cpp <- function(x, arg, dim, slice_indices) {
    .Call(`_rray_rray_subset_assign_cpp`, x, arg, dim, slice_indices)
}

